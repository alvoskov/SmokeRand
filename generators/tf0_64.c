/**
 * @file tf0_64.c
 * @brief A nonlinear PRNG based on Klimov-Shamir "crazy" T-function (tf0).
 * @details Klimov-Shamir T-function can be written as:
 * 
 * \f[
 * x_n = x_{n-1} + \left(x_{n-1}^2 \lor C\right) \mod 2^{64}
 * \f]
 * 
 * where the \f$C \mod 8 = 5\f$ or \f$C \mod 8 = 7\f$ condition must be
 * satisfied to achieve the full period. The `0x40000005` constant was
 * suggested by A.L. Voskov to pass BigCrush battery (TestU01).
 *
 * References:
 *
