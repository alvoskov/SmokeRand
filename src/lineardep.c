...
TestResults matrixrank_test(size_t n, unsigned int max_nbits,
    const GeneratorInfo *gi, void *state, const CallerAPI *intf)
{
    ...
    intf->printf("Matrix rank test\n");
    intf->printf("  n = %d. Number of matrices: %d\n", (int) n, nmat);
    for (int i = 0; i < nmat; i++) {
        ...
        if (max_nbits == 8) {
            ...
            for (size_t j = 0; j < mat_len * 4; j++) {
                a8[j] = gi->get_bits(state) & 0xFF;
            }
        } else if (gi->nbits == 32) {
            for (size_t j = 0; j < mat_len; j++) {
                a[j] = gi->get_bits(state);
            }
        } else {
            uint64_t *a64 = (uint64_t *) a;
            for (size_t j = 0; j < mat_len / 2; j++) {
                a64[j] = gi->get_bits(state);
            }
        }
    ...
    intf->printf("  %5s %10s %10s\n", "rank", "Oi", "Ei");
    for (int i = 0; i < 3; i++) {
        double Ei = pi[i] * nmat;
        ans.x += pow(Oi[i] - Ei, 2.0) / Ei;
        intf->printf("  %5d %10d %10.4g\n", i + (int) n - 2, Oi[i], Ei);
    }
    ans.p = exp(-0.5 * ans.x);
    intf->printf("  x = %g; p = %g\n\n", ans.x, ans.p);
    return ans;
